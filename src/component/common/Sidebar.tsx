      ...
      <div className="">
        ...
        <nav className="p-4">
          <ul className="space-y-1">
            {menuItems
              .filter((it) => !it.role || it.role === user.roleName)
              .map((item) => (
                <li key={item.link}>
                  <Link
                    to={item.link}
      ...
      <div className="absolute bottom-0 w-64 p-4 border-t border-r border-gray-200 bg-white">
        <div className="space-y-1">
          ...
          <button
            ...
            className="w-full cursor-pointer flex items-center gap-3 px-3 py-2.5 rounded-lg font-medium text-sm text-red-600 hover:bg-red-50 hover:text-red-700 transition-all duration-200 group"
          >
            <FontAwesomeIcon
              icon={faSignOutAlt}
              className="text-base text-red-500 group-hover:text-red-600"
            />
