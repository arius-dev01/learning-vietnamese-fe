  ...
  const handleSubmitAnswer = async (answerDTO: AnswerDTO) => {
    try {
      ...
      if (res.data.complete) {
        ...
        // Nếu đúng hết tất cả câu hỏi thì +20 điểm bonus
        const finalCorrectAnswers = res.data.correct
          ? correctAnswers + 1
          : correctAnswers;
        ...
        // Đợi 2s rồi mới hiện màn kết quả
      ...
      // Tự động next sau 2s - chặn submit trong thời gian này
      setIsTransitioning(true);
      setTimeout(() => {
        setIsTransitioning(false);
          ...
          <div className="space-y-6">
            <div className="flex items-center justify-between mb-6">
              <button
                ...
                className="flex cursor-pointer items-center space-x-2 text-gray-300 hover:text-white transition-colors p-2 rounded-lg hover:bg-white/10"
              >
                <span>←</span>
                <span>{t("Back")}</span>
              </button>
            ...
            <div className="border border-gray-600 rounded-xl p-4 space-y-4">
                  ...
                  <div className="flex flex-wrap gap-2 justify-center items-center min-h-[50px]">
                    {words.length === 0 ? (
                      <span className="text-gray-400 text-lg font-medium">
                        {t("Click words to build your sentence...")}
                      </span>
                    ) : (
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-3 justify-center p-4 bg-gray-800/30 rounded-xl border border-gray-700">
                    {availableWords.length === 0 ? (
                      <span className="text-gray-400 text-lg font-medium py-4">
                        All words used!
                      </span>
                    ) : (
              ...
              <div className="flex gap-4 justify-center">
                <button
                  className={`px-6 py-3 rounded-xl font-semibold transition-all shadow-lg transform hover:scale-105 ${
                    isTransitioning
                      ? "bg-gray-700 border border-gray-600 text-gray-400 cursor-not-allowed"
                      : "cursor-pointer bg-gradient-to-r from-gray-600 to-gray-700 hover:from-gray-700 hover:to-gray-800 text-white"
                  }`}
                  onClick={resetGame}
                ...
                <button
                  ...
                  disabled={words.length === 0 || isTransitioning}
                  className={`px-8 py-3 rounded-xl font-semibold transition-all shadow-lg transform hover:scale-105 ${
                    words.length === 0 || isTransitioning
                      ? "bg-gray-700 border border-gray-600 text-gray-400 cursor-not-allowed"
                      : "bg-gradient-to-r cursor-pointer from-green-600 to-green-700 hover:from-green-700 hover:to-green-800 text-white"
                  }`}
                >
                  {isTransitioning ? t("Please wait...") : t("Submit Answer")}
                </button>
              </div>
            </div>
            ...
            {result && (
              <div
                className={`p-4 rounded-xl text-center font-semibold transition-all duration-300 border backdrop-blur ${
                  isCorrect
